...
/*global module:false, htmlToText:false, Utils:false global:false*/
...
*/
  ...
  */
    ...
    this.preprocessInfo.html = this.preprocessInfo.html.replace(/<(img[^>]*)>/gi, "&lt;$1&gt;")
    ...
    if (this.checkingIfMarkdown) {
      ...
      this.preprocessInfo.html = this.preprocessInfo.html.replace(/<code\b.+?<\/code>/gi, "")
    } else {
    ...
    this.preprocessInfo.html = this.preprocessInfo.html
      ...
      // In the middle:
      .replace(/(<\/div>)((?!<div\b)[\s\S]+?)(<div\b[^>]*>)/gi, "$1<div>$2</div>$3")
      // At the end:
  ...
  MdhHtmlToText.prototype.excludeTagBlocks = function (
    ...
    if (ifHasAttribute) {
      openTagRegex = new RegExp(
        "<" + tagName + "\\b" + regexFiller + "\\b" + ifHasAttribute + "\\b" + regexFiller + ">",
        "i"
      )
    } else {
        ...
        if (depth > 0) {
          ...
          if (depth === 1) {
            ...
            this.preprocessInfo.html =
              ...
              "<br><br><br>" + // three empty lines is "guaranteed" to break a Markdown block
              placeholder +
              "<br><br><br>" +
...
*/
  function convertHTMLtoMarkdown(tag, html) {
    if (tag === "a") {
      var htmlToRestore = []
      html = html.replace(/(`+)[\s\S]+?\1/gi, function ($0) {
      ...
      We're going to use a regex that mimics a negative lookbehind. For details see:
      ...
      Here's an attempt at an explanation of the regex:
        ...
        )?                // capture group is optional so that we do the "negative" lookbehind --
                // that is, we can match links that are *not* preceded by the stuff we *don't* want
